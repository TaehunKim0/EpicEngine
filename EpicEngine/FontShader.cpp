...
bool FontShader::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../EpicEngine/font.vs", L"../EpicEngine/font.ps");
	if (!result)
	{
		return false;
	}
...
//Render 함수에서는 세이더의 인자를 설정하고 글꼴 셰이더를 사용하여 버퍼를 그려냅니다. 
//pixelColor 인자를 준다는 것 외에는 TextureShaderClass의 함수와 동일합니다.
...
//InitializeShader 함수는 글꼴 전용 정점/픽셀 셰이더를 인터페이스로 로드합니다.
bool FontShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	// 정점 입력 레이아웃 설명을 작성하십시오.
	// 이 설정은 ModelClass와 Shader의 VertexType 덕트와 일치해야 한다.
	...
	// 정점 셰이더 버퍼와 픽셀 셰이더 버퍼를 더 이상 필요하지 않으므로 분리하십시오.
...
//RenderShader 함수는 글꼴 셰이더를 이용하여 준비된 정점/인덱스 버퍼를 그립니다.
